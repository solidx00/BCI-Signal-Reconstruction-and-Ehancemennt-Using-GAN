...
def test_EEGwgan(generator, discriminator, noisy_dataloader, clean_dataloader, num_epochs=20, plot_interval=5):
    
    generator.load_state_dict(torch.load(save_path + r'\WGAN_generator_final_epoch.pth', weights_only=False))
    generator.eval()
    discriminator.eval()
    ...
    for epoch in range(num_epochs):
        ...
        for (noisy, _), (clean, _) in zip(noisy_dataloader, clean_dataloader):
            ...
            # Get critic output for the generated (fake) EEG signal
            fake_output = discriminator(denoised_eeg.detach().squeeze(0)).to(device)
        ...
        print(f"Epoch [{epoch}/{num_epochs}], Generator Loss:", loss_g.item())
...
clean_signals, noisy_signals, denoised_signals, generator_losses = test_EEGwgan(generator, discriminator, unhealthy_loader_test, healthy_loader_test, num_epochs=15)
